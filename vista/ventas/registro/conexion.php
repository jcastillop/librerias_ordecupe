...
class titulos
{
	...
	{
		$sql="select t.var_nom_tit,t.dec_preven_def_tit, t.dec_preven_sug_tit, t.int_cod_tit from T_titulos t where int_est_tit=1 and t.var_cod_bar_tit='".$id."'";
...
class clientes
{
	...
	{
		$sql="select var_rsoc_cli, var_ruc_cli, var_dir_cli,var_dist_cli,var_telf_cli from T_cliente where var_rsoc_cli='".$id."'";
		$res=mysql_query($sql,Conectar::con());
		while ($reg=mysql_fetch_assoc($res))
		{
			$this->cliente[]=$reg;
