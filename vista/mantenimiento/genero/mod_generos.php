...
if (isset($_GET["grabar"]) and $_GET["grabar"]=="si")
{
  ...
  $tra->edit_generos($_GET['id'],$_GET["var_nom_gen"],$_GET["var_des_gen"],$_GET["int_est_gen"],$user
	);
	exit;
}


$reg=$tra->get_generos_por_id($_GET["id"]);
		$var_nom_gen=$reg[0]["var_nom_gen"];
		$var_des_gen=$reg[0]["var_des_gen"];
		$int_est_gen=$reg[0]["int_est_gen"];
		?>
...
<!--LOGIN FORM-->
<form name="form1" class="login-form" action="mod_generos.php" method="get"  onSubmit="return formulario(this)">
    ...
    <div class="content">
      <table width="80%" border="1">
      	<tr>
          <td height="20">Nombre de Genero: </td>
          <td><input name="var_nom_gen" type="text" maxlength="10" style="width: 400px;" class="input username" id="var_nom_gen" value="<?php echo $var_nom_gen;?>" /> </td>
           </tr>
        <tr>
          <td height="20">Descripcion: </td>
          <td><input type="text" class="input username" style="width: 160px;"   name="var_des_gen" id="var_des_gen" value="<?php echo $var_des_gen;?>" /></td>
